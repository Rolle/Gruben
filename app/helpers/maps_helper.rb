module MapsHelper
	def popup(img)
		return '<p><h5>'+ img.adit.name + '</h5></p>'+ 
			...
			'<p>'+check_box_tag('ck_primary_'+img.id.to_s, img.id, img.primary, :data => {:remote=> true, :url => url_for(action: :primary, controller: :image_artifacts, id: img.id), id: img.id}) + 'Primär' + '</p>' +
